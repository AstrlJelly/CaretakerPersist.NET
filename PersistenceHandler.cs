...
namespace CaretakerNET.Persistence
{
    ...
    {
        public static readonly PersistenceHandler Instance = new();
        ...
        public async Task Load()
        {
            var loadGuilds = Task.Run(async () => {
                ...
                GuildData = GuildData.OrderBy(x => CaretakerCoreDiscordNET.Discord.Client.GetGuild(x.Key)?.GetUser(CARETAKER_ID)?.JoinedAt?.UtcTicks).ToDictionary();
